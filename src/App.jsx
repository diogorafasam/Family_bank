...
const SUPABASE_URL = "https://iwttpcfxxziivncwcger.supabase.co/rest/v1/";
...
export default function App(){
  ...
  const loadMember=async(uid)=>{
    const {data}=await sb.from("family_members").select("*").eq("user_id",uid).single();
    if(data){setMember(data);setFamilyId(data.family_id);}
  ...
  if(!familyId)return(<Ctx.Provider value={T}><style dangerouslySetInnerHTML={{__html:CSS}}/><Spinner/></Ctx.Provider>);
