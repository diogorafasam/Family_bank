...
// Substitui pelos teus valores: Supabase → Settings → API
...
const ACC_TYPES=[
  {id:"checking",label:"Conta à Ordem",icon:"🏦",color:"#0284C7"},
  {id:"savings",label:"Conta Poupança",icon:"💰",color:"#059669"},
  {id:"investment",label:"Investimentos",icon:"📈",color:"#7C3AED"},
  {id:"cash",label:"Dinheiro em Mão",icon:"💵",color:"#D97706"},
  {id:"crypto",label:"Criptomoedas",icon:"₿",color:"#F59E0B"},
  {id:"pension",label:"Fundo de Pensão",icon:"🎯",color:"#DB2777"},
  {id:"other",label:"Outro",icon:"📦",color:"#64748B"},
];
...
.btn.gr{background:${T.green}}.btn:disabled{opacity:.5;cursor:not-allowed;transform:none}
...
.bar{border-radius:4px 4px 0 0;transition:all .3s}.bar:hover{filter:brightness(1.1)}
...
@media(max-width:640px){
  .sidebar{display:none!important}.bot-nav{display:flex!important}
  .main-wrap{padding-bottom:70px!important}.hide-m{display:none!important}
  .stat{min-width:calc(50% - 7px)!important;padding:12px!important}
  .card{padding:13px!important;border-radius:12px!important}
  .modal{padding:18px!important;width:95%!important}.pad{padding:13px!important}
}
...
function Av({m,s=34}){
  ...
  const col=m.color||"#0284C7";
  return <div style={{width:s,height:s,background:col+"22",border:"2px solid "+col+"55",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:s*.34,fontWeight:700,color:col,flexShrink:0}}>{init}</div>;
}
function Prog({pct,color}){
  const T=useT();
  return <div className="track"><div className="fill" style={{"--w":Math.min(pct,100)+"%",background:pct>=100?"linear-gradient(90deg,"+T.green+","+T.accent+")":"linear-gradient(90deg,"+color+","+color+"cc)"}}/></div>;
}
function Toast({msg,icon,onClose}){
  useEffect(()=>{const t=setTimeout(onClose,3500);return()=>clearTimeout(t);},[]);
  return <div className="toast fi"><div style={{display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:20}}>{icon}</span><span style={{fontSize:13,fontWeight:600,flex:1}}>{msg}</span><button className="ibtn" onClick={onClose}>×</button></div></div>;
}
...
function Mdl({title,onClose,onSave,saveLabel,children}){
  return <div className="modal-bg" onClick={onClose}>
    <div className="modal" onClick={e=>e.stopPropagation()}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <h3 style={{fontSize:17,fontWeight:700}}>{title}</h3>
        <button className="ibtn" onClick={onClose} style={{fontSize:22}}>×</button>
      </div>
      {children}
      <div style={{display:"flex",gap:9,marginTop:20}}>
        <button className="gbtn" onClick={onClose} style={{flex:1}}>Cancelar</button>
        <button className="btn" onClick={onSave} style={{flex:2,justifyContent:"center"}}>{saveLabel||"Guardar"}</button>
      </div>
    </div>
  </div>;
}
...
function AuthScreen(){
  ...
  const doRegister=async()=>{
    ...
    if(fe){setErr(fe.message);setLoading(false);return;}
    await sb.from("family_members").insert({family_id:fam.id,user_id:data.user.id,name:name.trim(),role:"admin",color:COLORS5[0]});
    setLoading(false);
  };
  ...
  const doJoin=async()=>{
    ...
    if(fe||!fam){setErr("Código inválido. Verifica e tenta novamente.");setLoading(false);return;}
    const {data,error}=await sb.auth.signUp({email,password:pass});
    if(error){setErr(error.message);setLoading(false);return;}
    await sb.from("family_members").insert({family_id:fam.id,user_id:data.user.id,name:name.trim(),role:"member",color:COLORS5[1]});
    setLoading(false);
  };
    ...
    <div style={{width:"100%",maxWidth:400}}>
      ...
      <div style={{background:T.surface,border:"1px solid "+T.border,borderRadius:20,padding:26,boxShadow:"0 4px 24px "+T.shadow}}>
        <div style={{display:"flex",gap:3,marginBottom:20,background:T.elevated,borderRadius:11,padding:3}}>
          {TABS.map(t=><button key={t.id} onClick={()=>{setMode(t.id);setErr("");}} style={{flex:1,padding:"7px 0",borderRadius:9,border:"none",cursor:"pointer",fontSize:12,fontWeight:600,background:mode===t.id?T.surface:"transparent",color:mode===t.id?T.accent:T.text2,transition:"all .2s",boxShadow:mode===t.id?"0 1px 4px "+T.shadow:"none"}}>{t.l}</button>)}
        </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {(mode==="register"||mode==="invite")&&<Fld label="O teu nome"><input className="inp" placeholder="Ex: Ana Silva" value={name} onChange={e=>setName(e.target.value)}/></Fld>}
          {mode==="register"&&<Fld label="Nome da família"><input className="inp" placeholder="Ex: Família Silva" value={famName} onChange={e=>setFamName(e.target.value)}/></Fld>}
          {mode==="invite"&&<Fld label="Código de convite"><input className="inp" placeholder="Ex: FAM2026AB" value={invCode} onChange={e=>setInvCode(e.target.value)} style={{fontFamily:"monospace",letterSpacing:3,textTransform:"uppercase"}}/></Fld>}
        ...
        {mode==="login"&&<p style={{textAlign:"center",fontSize:12,color:T.text2,marginTop:12}}>Não tens conta? Usa os separadores acima.</p>}
      </div>
    </div>
  </div>;
}

// ─── ONBOARDING: saldo inicial ───────────────────────────────────────────────
function Onboarding({familyId,onDone}){
  const T=useT();
  const [accs,setAccs]=useState([
    {id:1,type:"checking",label:"",balance:""},
    {id:2,type:"savings",label:"",balance:""},
  ]);
  ...
  const finish=async()=>{
    ...
    setSaving(true);
    await sb.from("accounts").insert(valid.map(a=>({family_id:familyId,type:a.type,label:a.label.trim(),balance:parseFloat(a.balance)})));
    onDone();
  };
  ...
  return <div style={{minHeight:"100vh",background:T.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
    <div style={{width:"100%",maxWidth:520}}>
      <div style={{textAlign:"center",marginBottom:22}}>
        ...
        <p style={{fontSize:14,color:T.text2,marginTop:6,lineHeight:1.6}}>Introduz <strong style={{color:T.text}}>todo o dinheiro que tens agora</strong>.<br/>Este será o teu <strong style={{color:T.accent}}>saldo inicial</strong>.</p>
      </div>
          ...
          {accs.map(acc=>{
            ...
            return <div key={acc.id} style={{background:T.elevated,border:"1px solid "+T.border,borderRadius:13,padding:13}}>
              ...
              <div style={{display:"flex",gap:9}}>
                <div style={{flex:2}}><div style={{fontSize:10,color:T.text2,marginBottom:4}}>DESCRIÇÃO</div><input className="inp" placeholder={"Ex: "+at.label+" CGD"} value={acc.label} onChange={e=>upd(acc.id,"label",e.target.value)}/></div>
...
function useFamily(familyId){
  ...
  useEffect(()=>{
    ...
    const ch=sb.channel("fb-"+familyId)
      ...
      .on("postgres_changes",{event:"*",schema:"public",table:"family_members",filter:"family_id=eq."+familyId},load)
      .subscribe();
...
function Dashboard({d}){
  ...
  const pat=accounts.reduce((s,a)=>s+parseFloat(a.balance),0);
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:18}}>
    <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
      {[{l:"Patrimônio Total",v:fmt(pat),col:T.accent,i:"🏦"},{l:"Receitas Mês",v:fmt(inc),col:T.green,i:"📥"},{l:"Despesas Mês",v:fmt(exp),col:T.red,i:"📤"},{l:"Taxa Poupança",v:rate+"%",col:T.accent,i:"📊"}].map(s=>
    ...
    <div style={{display:"flex",gap:14,flexWrap:"wrap"}}>
      <div className="card fu" style={{flex:"2 1 300px"}}>
        ...
        {recent.length===0
          ?<div style={{textAlign:"center",padding:"24px",color:T.text2,fontSize:13}}>Sem transações ainda. Adiciona a primeira!</div>
          :recent.map(t=>{const cat=gc(t.category),m=gm(t.user_id);return <div key={t.id} className="tr">
            ...
            <div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.description}</div><div style={{fontSize:11,color:T.text2}}>{cat.label} · {m.name}</div></div>
      ...
      <div className="card fu" style={{flex:"1 1 190px"}}>
        <div style={{fontWeight:600,fontSize:14,marginBottom:12}}>Por Categoria</div>
        {catTots.length===0?<div style={{fontSize:13,color:T.text2,textAlign:"center",padding:"20px 0"}}>Sem despesas ainda</div>
          :catTots.slice(0,5).map(cat=><div key={cat.id} style={{marginBottom:9}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:4,fontSize:12}}><span>{cat.icon} {cat.label}</span><span className="mono" style={{color:T.text2}}>{fmt(cat.total)}</span></div><Prog pct={exp>0?(cat.total/exp)*100:0} color={cat.color||"#64748B"}/></div>)}
      </div>
    </div>
    {goals.length>0&&<div className="card fu">
      <div style={{fontWeight:600,fontSize:14,marginBottom:12}}>Objetivos de Poupança</div>
      <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
        {goals.slice(0,3).map(g=>{const pct=parseFloat(g.current_amount)/parseFloat(g.target)*100;return <div key={g.id} style={{flex:"1 1 180px",background:T.elevated,border:"1px solid "+T.border,borderRadius:12,padding:13}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div style={{display:"flex",alignItems:"center",gap:7}}><span style={{fontSize:18}}>{g.icon}</span><div style={{fontWeight:600,fontSize:13}}>{g.name}</div></div><span className="mono" style={{fontWeight:700,color:g.color||T.accent,fontSize:13}}>{pct.toFixed(0)}%</span></div>
          <Prog pct={pct} color={g.color||T.accent}/>
          <div style={{display:"flex",justifyContent:"space-between",marginTop:6,fontSize:11,color:T.text2}}><span>{fmt(parseFloat(g.current_amount))}</span><span>{fmt(parseFloat(g.target))}</span></div>
        </div>;})}
      </div>
    </div>}
    {members.length>0&&<div className="card fu">
      <div style={{fontWeight:600,fontSize:14,marginBottom:12}}>Gastos por Membro</div>
      <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
        {members.map(m=>{const tot=tx.filter(t=>t.user_id===m.user_id&&t.type==="expense").reduce((a,t)=>a+parseFloat(t.amount),0);const pct=exp>0?(tot/exp)*100:0;return <div key={m.id} style={{flex:"1 1 140px",background:T.elevated,border:"1px solid "+T.border,borderRadius:12,padding:13}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}><Av m={m} s={30}/><div><div style={{fontSize:13,fontWeight:600}}>{m.name}</div><div style={{fontSize:10,color:T.text2}}>{m.role==="admin"?"Admin":"Membro"}</div></div></div><div className="mono" style={{fontSize:16,fontWeight:700,color:m.color||T.accent}}>{fmt(tot)}</div><div style={{marginTop:7}}><Prog pct={pct} color={m.color||T.accent}/><div style={{fontSize:10,color:T.text2,marginTop:3}}>{pct.toFixed(1)}%</div></div></div>;})}
      </div>
    </div>}
  </div>;
}
...
function Transactions({d,familyId,userId,toast}){
  const T=useT();
  const {transactions:tx,categories:cats,members}=d;
  ...
  const save=async(f)=>{
    const row={family_id:familyId,user_id:userId,type:f.type,amount:parseFloat(f.amount),category:f.category,description:f.description,date:f.date,recurring:f.recurring||false};
    if(f.id)await sb.from("transactions").update(row).eq("id",f.id);
    else await sb.from("transactions").insert({...row,id:uid()});
    toast(f.type==="income"?"Receita: "+fmt(parseFloat(f.amount)):"Despesa: "+fmt(parseFloat(f.amount)),f.type==="income"?"📥":"📤");
    setMdl(null);
  };
  const del=async(id)=>{await sb.from("transactions").delete().eq("id",id);toast("Transação eliminada","🗑️");};
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <div className="card" style={{padding:0,overflow:"hidden"}}>
      <table className="tbl">
        <thead><tr><th>Descrição</th><th className="hide-m">Categoria</th><th className="hide-m">Por</th><th className="hide-m">Data</th><th>Valor</th><th style={{width:72}}>Ações</th></tr></thead>
        <tbody>
          {list.map(t=>{const cat=gc(t.category),m=gm(t.user_id);return <tr key={t.id}>
            <td><div style={{display:"flex",alignItems:"center",gap:9}}><div style={{width:30,height:30,background:(cat.color||"#64748B")+"18",borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0}}>{cat.icon}</div><span style={{fontSize:13,fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:150}}>{t.description}</span></div></td>
            <td className="hide-m" style={{fontSize:12,color:T.text2}}>{cat.label}</td>
            ...
            <td><div style={{display:"flex",gap:2}}><button className="ibtn edit" onClick={()=>setMdl(t)}>✏️</button><button className="ibtn del" onClick={()=>del(t.id)}>🗑</button></div></td>
          </tr>;})}
        </tbody>
      </table>
      {list.length===0&&<div style={{textAlign:"center",padding:"32px",color:T.text2}}><div style={{fontSize:30,marginBottom:8}}>📭</div>Sem transações</div>}
    </div>
    {mdl&&<TxModal init={mdl==="new"?null:mdl} cats={cats} onClose={()=>setMdl(null)} onSave={save}/>}
  </div>;
}

function TxModal({init,cats,onClose,onSave}){
  const T=useT();
  const blank={type:"expense",amount:"",category:cats[0]?.id||"",description:"",date:new Date().toISOString().slice(0,10),recurring:false};
  const [f,setF]=useState(init?{...init,amount:String(parseFloat(init.amount))}:blank);
  ...
  return <Mdl title={init?"Editar Transação":"Nova Transação"} onClose={onClose} onSave={()=>{if(!f.amount||!f.description)return;onSave(f);}} saveLabel={init?"Guardar":"Adicionar"}>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      <Fld label="Descrição"><input className="inp" placeholder="Ex: Compras semanais..." value={f.description} onChange={e=>h("description",e.target.value)}/></Fld>
...
function Accounts({d,familyId,toast}){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      <div><h2 style={{fontSize:17,fontWeight:600}}>Contas e Patrimônio</h2><p style={{fontSize:13,color:T.text2,marginTop:3}}>Sincronizado em tempo real</p></div>
      <button className="btn" onClick={()=>setMdl("new")}>+ Nova conta</button>
    </div>
    <div style={{background:"linear-gradient(135deg,"+T.accent+","+T.green+")",borderRadius:18,padding:"22px 24px",color:"#fff"}}>
      <div style={{fontSize:11,opacity:.8,marginBottom:5,textTransform:"uppercase",letterSpacing:".8px"}}>Patrimônio Total</div>
      <div className="mono" style={{fontSize:32,fontWeight:900,marginBottom:3}}>{fmt(total)}</div>
      <div style={{fontSize:13,opacity:.75}}>{accounts.length} conta{accounts.length!==1?"s":""} registada{accounts.length!==1?"s":""}</div>
    </div>
    {accounts.length===0
      ?<div className="card" style={{textAlign:"center",padding:"36px 20px",color:T.text2}}><div style={{fontSize:36,marginBottom:10}}>🏦</div><p style={{marginBottom:14}}>Adiciona as tuas contas.</p><button className="btn" onClick={()=>setMdl("new")} style={{margin:"0 auto"}}>+ Adicionar</button></div>
      :<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(255px,1fr))",gap:13}}>
        {accounts.map(acc=>{
          const at=ACC_TYPES.find(t=>t.id===acc.type)||ACC_TYPES[0];
          const pct=total>0?(parseFloat(acc.balance)/total)*100:0;
          return <div key={acc.id} className="card fu" style={{position:"relative",overflow:"hidden"}}>
            ...
            <div className="mono" style={{fontSize:24,fontWeight:800,color:at.color,marginBottom:8}}>{fmt(parseFloat(acc.balance))}</div>
            <div style={{height:5,background:T.trackBg,borderRadius:3,overflow:"hidden",marginBottom:5}}><div style={{height:"100%",width:pct+"%",background:at.color,borderRadius:3}}/></div>
            <div style={{fontSize:11,color:T.text2}}>{pct.toFixed(1)}% do patrimônio total</div>
          </div>;
        })}
      </div>
    }
...
function AccModal({init,onClose,onSave}){
  ...
  return <Mdl title={init?"Editar Conta":"Nova Conta"} onClose={onClose} onSave={()=>{if(!f.label||!f.balance)return;onSave(f);}} saveLabel={init?"Guardar":"Adicionar"}>
    <div style={{display:"flex",flexDirection:"column",gap:13}}>
      <Fld label="Tipo de conta"><select className="sel" value={f.type} onChange={e=>h("type",e.target.value)}>{ACC_TYPES.map(t=><option key={t.id} value={t.id}>{t.icon} {t.label}</option>)}</select></Fld>
      <Fld label="Nome / Descrição"><input className="inp" placeholder={"Ex: "+at.label+" CGD"} value={f.label} onChange={e=>h("label",e.target.value)}/></Fld>
      ...
      <div style={{padding:"10px 13px",background:T.accentS,borderRadius:9,border:"1px solid "+T.accent+"33",fontSize:12,color:T.text2,lineHeight:1.6}}>
        {init?"Edita o saldo para corrigir. As próximas transações irão atualizá-lo.":"Introduz o saldo que tens hoje nesta conta."}
      </div>
    </div>
  </Mdl>;
}
...
function Goals({d,familyId,toast}){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:15}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      <div><h2 style={{fontSize:17,fontWeight:600}}>Objetivos de Poupança</h2><p style={{fontSize:13,color:T.text2,marginTop:3}}>Visível por toda a família em tempo real</p></div>
      <button className="btn" onClick={()=>setMdl("new")}>+ Novo</button>
    </div>
    {goals.length===0&&<div className="card" style={{textAlign:"center",padding:"36px",color:T.text2}}><div style={{fontSize:36,marginBottom:10}}>🎯</div><p style={{marginBottom:14}}>Cria o primeiro objetivo de poupança.</p><button className="btn" onClick={()=>setMdl("new")} style={{margin:"0 auto"}}>+ Criar objetivo</button></div>}
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(255px,1fr))",gap:13}}>
      {goals.map(g=>{
...
function GoalModal({init,onClose,onSave}){
  ...
  return <Mdl title={init?"Editar Objetivo":"Novo Objetivo"} onClose={onClose} onSave={()=>{if(!f.name||!f.target)return;onSave(f);}} saveLabel={init?"Guardar":"Criar"}>
    <div style={{display:"flex",flexDirection:"column",gap:13}}>
      ...
      <Fld label="Nome"><input className="inp" placeholder="Ex: Férias em Paris..." value={f.name} onChange={e=>h("name",e.target.value)}/></Fld>
      <div style={{display:"flex",gap:11}}>
...
function Members({d}){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(230px,1fr))",gap:12}}>
      {members.map(m=><div key={m.id} className="card fu"><div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}><Av m={m} s={46}/><div><div style={{fontWeight:700,fontSize:15}}>{m.name}</div><span className="badge" style={{background:m.role==="admin"?T.accentS:T.elevated,color:m.role==="admin"?T.accent:T.text2,marginTop:4}}>{m.role==="admin"?"⭐ Admin":"👤 Membro"}</span></div></div></div>)}
    </div>
    <div className="card">
      <div style={{fontWeight:600,marginBottom:8}}>📩 Código de convite</div>
      <p style={{fontSize:13,color:T.text2,marginBottom:13,lineHeight:1.6}}>Partilha este código com quem queres convidar. Na app escolhem <strong style={{color:T.text}}>"Tenho convite"</strong> e inserem o código.</p>
...
function Settings({user,dark,setDark,onLogout}){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    <div><h2 style={{fontSize:17,fontWeight:600}}>Definições</h2><p style={{fontSize:13,color:T.text2,marginTop:3}}>Conta e preferências</p></div>
    <div className="card"><div style={{fontWeight:600,fontSize:14,marginBottom:14}}>Aparência</div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}><span style={{fontSize:14}}>Modo Escuro</span><Tog on={dark} toggle={()=>setDark(p=>!p)}/></div>
    </div>
    <div className="card"><div style={{fontWeight:600,fontSize:14,marginBottom:14}}>Segurança</div>
    ...
    <div className="card"><div style={{fontWeight:600,fontSize:14,marginBottom:14}}>Conta</div>
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    const {data:{subscription}}=sb.auth.onAuthStateChange((_,session)=>{
      if(session?.user){setUser(session.user);loadMember(session.user.id);}
      else{setUser(null);setMember(null);setFamilyId(null);setAuthLoading(false);}
    });
  ...
  const loadMember=async(userId)=>{
    try{
      const {data,error}=await sb.from("family_members").select("*").eq("user_id",userId);
  ...
  if(!familyId)return(<Ctx.Provider value={T}><style dangerouslySetInnerHTML={{__html:CSS}}/><div style={{minHeight:"100vh",background:T.bg,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16,padding:20}}><div style={{fontSize:40}}>⚠️</div><div style={{fontSize:16,fontWeight:600,color:T.text,textAlign:"center"}}>Conta criada!</div><div style={{fontSize:14,color:T.text2,textAlign:"center",lineHeight:1.6}}>A tua conta foi criada mas ainda não está ligada a uma família.<br/>Tenta terminar sessão e entrar novamente.</div><button onClick={logout} style={{marginTop:8,padding:"10px 24px",background:T.accent,color:"#fff",border:"none",borderRadius:10,fontSize:14,fontWeight:600,cursor:"pointer"}}>Terminar sessão</button></div></Ctx.Provider>);
      ...
      <div className="sidebar" style={{width:sidebar?220:62}}>
        <div style={{padding:"16px 12px",borderBottom:"1px solid "+T.border}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:36,height:36,flexShrink:0,background:"linear-gradient(135deg,"+T.accent+","+T.green+")",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,fontWeight:900,color:"#fff",boxShadow:"0 4px 14px "+T.accent+"44"}}>F</div>
            {sidebar&&<div style={{overflow:"hidden"}}><div style={{fontWeight:800,fontSize:14,color:T.text,whiteSpace:"nowrap"}}>FamilyBank</div><div style={{fontSize:11,color:T.text2,marginTop:1,display:"flex",alignItems:"center",gap:5}}><span className="dot"/><span>Tempo real</span></div></div>}
          </div>
        </div>
        ...
        <div style={{padding:"7px",borderTop:"1px solid "+T.border}}>
          <button className="gbtn" onClick={()=>setSidebar(p=>!p)} style={{width:"100%",justifyContent:"center",padding:"7px",fontSize:12}}>{sidebar?"◀ Recolher":"▶"}</button>
        </div>
      </div>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
        <div className="hdr" style={{padding:"10px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
          <div style={{minWidth:0}}>
            <h1 style={{fontSize:15,fontWeight:700,display:"flex",alignItems:"center",gap:6,color:T.text}}><span>{cur?.icon}</span><span style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{cur?.label}</span></h1>
            <div className="hide-m" style={{fontSize:11,color:T.text2,marginTop:1}}>{d.family?.name||"—"}</div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:7,flexShrink:0}}>
            ...
            {tab==="transactions"&&<button className="btn hide-m" onClick={()=>setTab("transactions")}>+ Adicionar</button>}
          </div>
        </div>
